...
export default function AdminDashboard({ user, onLogout, onBackToMain }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      {/* Header */}
      <div className="bg-white/5 backdrop-blur-xl border-b border-white/10 p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <button
                onClick={onBackToMain}
                className="flex items-center text-slate-300 hover:text-white transition-colors text-sm sm:text-base"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <span className="text-slate-300 text-sm sm:text-base">Welcome, {user?.name}</span>
              <button
                onClick={onLogout}
                className="px-3 py-2 sm:px-4 bg-red-600/20 border border-red-500/30 rounded-xl text-red-300 hover:bg-red-600/30 hover:text-white transition-all duration-300 text-sm sm:text-base"
              >
                Logout
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-2 mb-6">
          <div className="flex overflow-x-auto scrollbar-hide space-x-2 pb-1">
            {[
              { id: 'overview', label: 'Overview', shortLabel: 'Overview', icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z' },
              { id: 'users', label: 'User Management', shortLabel: 'Users', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z' },
              { id: 'attendance', label: 'Attendance Records', shortLabel: 'Attendance', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
              { id: 'reports', label: 'Reports & Analytics', shortLabel: 'Reports', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' }
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center space-x-2 px-3 sm:px-4 py-2 rounded-xl font-medium transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
                  activeTab === tab.id
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/5 rounded-xl p-4 border border-white/10">
              ...
              <div className="space-y-2">
                ...
                {/* Removed Admin Access section as per user request */}
              </div>
            </div>
                <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          ...
          {activeTab === 'attendance' && (
            <AttendanceManagementScreen embedded={true} />
          )}
